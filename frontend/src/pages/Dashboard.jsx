  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div ref={statsContainerRef} className="grid md:grid-cols-3 gap-6 mb-8">
        {[
          { icon: BookOpen, label: 'Total Materi', value: materiList.length, colorClass: 'text-indigo-600 dark:text-indigo-400', bgClass: 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-100/50 dark:border-indigo-900/30' },
          { icon: CheckCircle, label: 'Selesai Dipelajari', value: progress?.selesai || 0, colorClass: 'text-emerald-600 dark:text-emerald-400', bgClass: 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100/50 dark:border-emerald-900/30' },
          { icon: BarChart3, label: 'Kemajuan Belajar', value: `${progress?.progress_persen || 0}%`, colorClass: 'text-cyan-600 dark:text-cyan-400', bgClass: 'bg-cyan-50 dark:bg-cyan-950/40 border-cyan-100/50 dark:border-cyan-900/30' },
        ].map((s, i) => (
          <div key={i} className="card p-6 flex items-center gap-4 stats-card opacity-0">
            <div className={`w-12 h-12 rounded-2xl ${s.bgClass} border flex items-center justify-center shrink-0`}>
            ...
            <div>
              ...
              <div className={`stat-value text-3xl font-black mt-0.5 ${s.colorClass}`}>{s.value}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {progress && (
        <div ref={progressCardRef} className="card p-6 mb-10 border border-slate-200/80 dark:border-slate-800/60 progress-card opacity-0">
          <div className="flex items-center justify-between mb-4.5">
            ...
            <span className="font-display font-black text-indigo-600 dark:text-indigo-400 text-2xl bg-indigo-50 dark:bg-indigo-950/40 px-3.5 py-1 rounded-2xl border border-indigo-100/50 dark:border-indigo-900/30">{progress.progress_persen}%</span>
          </div>
                ...
                <div>
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-3.5 mb-4">
                      <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800/80 flex items-center justify-center shadow-sm border border-slate-100 dark:border-slate-800/60 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                        {(() => {
                 ...
                 <div className="px-6 pb-6 pt-4 border-t border-slate-100/80 dark:border-slate-800/80 flex items-center justify-between bg-slate-50/40 dark:bg-slate-900/40">
                  ...
                  <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-800 group-hover:bg-[var(--color-brand-deep)] flex items-center justify-center shadow-sm border border-slate-100 dark:border-slate-800/60 group-hover:border-transparent transition-all duration-300">

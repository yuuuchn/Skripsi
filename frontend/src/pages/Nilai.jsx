  ...
  const achievementsList = [
    {
      ...
      bg: 'bg-amber-50 dark:bg-amber-900/40 border-amber-100/50 dark:border-amber-800/40',
      color: 'text-amber-500 dark:text-amber-400 fill-amber-400 border-0'
    },
    {
      title: 'Siswa Teladan',
      ...
      bg: 'bg-yellow-50 dark:bg-yellow-900/40 border-yellow-100/50 dark:border-yellow-800/40',
      color: 'text-yellow-600 dark:text-yellow-400 fill-yellow-400 border-0'
    },
    {
      title: 'Ahli Jaringan',
    ...
    },
    {
      ...
      bg: 'bg-emerald-50 dark:bg-emerald-900/40 border-emerald-100/50 dark:border-emerald-800/40',
      color: 'text-emerald-600 dark:text-emerald-400 fill-emerald-400 border-0'
    }
  ];
    ...
    <div ref={containerRef} className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 nilai-header-section opacity-0">
        <h1 className="font-display text-3xl md:text-4xl font-extrabold text-[var(--color-text)] flex items-center gap-3 tracking-tight">
          <div className="w-10 h-10 rounded-2xl bg-amber-50 dark:bg-amber-900/40 border border-amber-100 dark:border-amber-800/50 flex items-center justify-center text-amber-500 dark:text-amber-400 shadow-sm shrink-0">
      ...
      {activeTab === 'personal' ? (
        nilaiList.length === 0 ? (
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-5 mb-8 nilai-stats-container">
              {[
                { icon: BarChart3, label: 'Rata-rata', value: rataRata, color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-50 border-indigo-100/50 dark:bg-indigo-900/30 dark:border-indigo-800/40' },
                { icon: Medal, label: 'Tertinggi', value: tertinggi, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 border-emerald-100/50 dark:bg-emerald-900/30 dark:border-emerald-800/40' },
                { icon: FileText, label: 'Kuis Selesai', value: nilaiList.length, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 border-amber-100/50 dark:bg-amber-900/30 dark:border-amber-800/40' },
              ].map((s, i) => (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {achievementsList.map((ach, i) => {
                  ...
                  return (
                    <div 
                      key={i} 
                      className={`card p-4.5 text-center flex flex-col items-center justify-between border-slate-200/50 dark:border-slate-700/50 shadow-sm transition-all duration-300 nilai-achievement-card opacity-0 ${
                        ach.unlocked 
                          ? 'bg-gradient-to-b from-white dark:from-slate-800 to-slate-50/20 dark:to-slate-800/20 border-indigo-100/60 dark:border-indigo-800/40 hover:shadow-md hover:shadow-indigo-500/5 hover:-translate-y-0.5' 
                          : 'opacity-55 bg-slate-50/30 dark:bg-slate-800/30 border-slate-200/30 dark:border-slate-700/30 select-none'
                      }`}
                    >
                      <div className="relative">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-3.5 shadow-sm ${
                          ach.unlocked ? ach.bg : 'bg-slate-100 dark:bg-slate-800 text-slate-400 shadow-none border border-slate-200/20 dark:border-slate-700/20'
                        }`}>
                      ...
                      <div>
                        <h4 className={`font-display font-bold text-[11px] tracking-tight ${ach.unlocked ? 'text-slate-800 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500'}`}>{ach.title}</h4>
                        <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-1 leading-normal font-medium">{ach.desc}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="card overflow-hidden border-slate-200/60 dark:border-slate-700/60 shadow-sm nilai-leaderboard-card opacity-0">
              <div className="p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest font-mono">Daftar Nilai</span>
              </div>
              ...
              <div className="divide-y divide-slate-100/80 dark:divide-slate-700/80">
                {nilaiList.map((n, idx) => {
                  ...
                  return (
                    <div key={idx} className="px-5 py-4.5 flex items-center justify-between hover:bg-slate-50/40 dark:hover:bg-slate-800/40 transition-colors nilai-leaderboard-item opacity-0">
                      ...
                      <div className="flex items-center gap-2 pr-2">
                        <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
                          ...
                          <span className={`stat-value text-lg font-black ${pct >= 80 ? 'text-emerald-600 dark:text-emerald-400' : pct >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
                            {pct}
                          </span>
                        </div>
                      </div>
        ...
        <div className="card overflow-hidden border-slate-200/60 dark:border-slate-700/60 shadow-sm nilai-scoreboard-card opacity-0">
          <div className="p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex items-center justify-between">
            <h2 className="font-display font-extrabold text-base text-[var(--color-text)] flex items-center gap-2">
              <Trophy className="w-5 h-5 text-amber-500 fill-amber-400" />
              Peringkat 10 Besar Kelas
            </h2>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest font-mono">Papan Juara</span>
          </div>
          ...
          {leaderboardLoading ? (
            ...
            <div className="divide-y divide-slate-100/80 dark:divide-slate-700/80">
              {leaderboard.map((item, idx) => {
                ...
                return (
                  <div key={idx} className={`px-5 py-4.5 flex items-center justify-between hover:bg-slate-50/40 dark:hover:bg-slate-800/40 transition-colors nilai-scoreboard-item opacity-0 ${
                    idx === 0 ? 'bg-amber-50/10 dark:bg-amber-900/10' : ''
                  }`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 mt-0.5">{item.kelas || 'Siswa'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 px-3.5 py-1.5 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm">
                        ...
                        <div>
                          <span className="stat-value text-base font-black text-indigo-600 dark:text-indigo-400">{item.total_skor}</span>
                          <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 ml-1">total</span>
                        </div>
                      </div>
                    </div>
                  </div>
